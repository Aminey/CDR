...
dtheta_dz_1.sea(j,k) = (1/(2*A1*G)) * (q01_sea*(sum_term_a1(j,k)) + (q01_sea-q02_sea)*(caps.y_low_c - caps.y_upp_c)/spar.t + ((Sy.sea(j,k)*Ixy - Sx.sea(j,k)*Ixx)/(Ixx*Iyy-Ixy^2))*(sum_term_a2.sea(j,k)) + ((Sx.sea(j,k)*Ixy-Sy.sea(j,k)*Iyy)/(Ixx*Iyy-Ixy^2))*(sum_term_a3.sea(j,k)));
dtheta_dz_1.alt(j,k) = (1/(2*A1*G)) * (q01_sea*(sum_term_a1(j,k)) + (q01_alt-q02_alt)*(caps.y_low_c - caps.y_upp_c)/spar.t + ((Sy.alt(j,k)*Ixy - Sx.alt(j,k)*Ixx)/(Ixx*Iyy-Ixy^2))*(sum_term_a2.alt(j,k)) + ((Sx.alt(j,k)*Ixy-Sy.alt(j,k)*Iyy)/(Ixx*Iyy-Ixy^2))*(sum_term_a3.alt(j,k)));
...
dtheta_dz_2.sea(j,k) = q02_sea*(sum_term_b1(j,k)) + (q02_sea-q01_sea)*(caps.y_upp_c-caps.y_low_c)/spar.t + q02_sea*(caps.y_low_c - caps.y_upp_c)/(spar.t) + ((Sy.sea(j,k)*Ixy - Sx.sea(j,k)*Ixx)/(Ixx*Iyy - Ixy^2))*(sum_term_b2.sea(j,k)) + ((Sx.sea(j,k)*Ixy - Sy.sea(j,k)*Iyy)/(Ixx*Iyy-Ixy^2))*(sum_term_b3.sea(j,k));
dtheta_dz_2.alt(j,k) = q02_alt*(sum_term_b1(j,k)) + (q02_alt-q01_alt)*(caps.y_upp_c-caps.y_low_c)/spar.t + q02_alt*(caps.y_low_c - caps.y_upp_c)/(spar.t) + ((Sy.sea(j,k)*Ixy - Sx.sea(j,k)*Ixx)/(Ixx*Iyy - Ixy^2))*(sum_term_b2.alt(j,k)) + ((Sx.sea(j,k)*Ixy - Sy.sea(j,k)*Iyy)/(Ixx*Iyy-Ixy^2))*(sum_term_b3.alt(j,k));
...
for i = spar.i_CCW(3):length(x)
    ...
    q.sea_spar(j,k) = q01.sea - q02.sea;
    q.alt_spar(j,k) = q01.alt - q02.alt;
